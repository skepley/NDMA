...
def test_HillModel():
    ...
    y = f(x, p)
    # print(y)
    assert np.abs(-gammaVals[0] * x[0] + H(x[0]) + H(x[1]) + H(x[2])- y[0])<10**-6
    assert np.abs(-gammaVals[1] * x[1] + H(x[0]) + H(x[2])- y[1])<10**-6
    assert np.abs(-gammaVals[2] * x[2] + H(x[0])- y[2])<10**-6
    ...
    assert np.linalg.norm(yx - sol_array)<10**-5
